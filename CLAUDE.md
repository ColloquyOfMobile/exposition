...
- **Units in `params.json`**: `"dxl origin"` is in raw servo units — it is the reading a body gives when it points where it should. *Everything else is in degrees of the body*: the bar's `interaction_origins`, the simulator's `near origin threshold` (per kind, since the reductions differ).
