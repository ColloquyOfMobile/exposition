...
The snapshot dict is the **whole contract** between the tree and the page, and `colloquy/ui/leaves.py` is where its vocabulary is defined — one constructor per kind of thing the page can draw (`value`, `html`, `chart`, `svg`, `pre`, `editor`), plus `into(states, path)` for a node with several readings to show. Build leaves with those rather than hand-writing `{"path": ..., "name": ..., "value": ...}`; a kind that isn't in `leaves.KINDS` is one the renderer will not draw.

**Setting a value from the page** is `leaves.editable(path, key, value, hint=...)`: it draws the reading plus a box, and posts what is typed to a `commit` command the node must register (`self["commit"] = self.commit`). `commit` takes the raw string and is expected to **raise** on anything it won't accept — `WSGI2._parse_post` turns that into a log line and leaves the value alone. It must never be allowed to escape: `Server2.wsgi()` treats an unhandled exception in a request as an emergency stop, so a typo in a box would otherwise halt the installation. Two things in a snapshot are *not* leaves: child nodes (from `snapshot_children`) and commands (bare callables, rendered as links that call them through the `call` path segment).
