...
Each male continuously blinks his ring in a repeating 10-step, 0.5s-per-step on/off sequence — a morse-code-like signal encoding (a) which male he is (`male1`/`male2`) and (b) which drive state he currently wants (`O`, `P`, both, or neither). The 4 reference sequences per male live in `Colloquy.light_patterns` (`colloquy/__init__.py`). Write side: `hardware/male/__init__.py`'s `Male.get_blink_pattern()` picks the sequence for `self.drives.which_is_frustated()`, converted to a `deque` per male at construction so `hardware/male/search/blink/__init__.py`'s `Blink.loop()` can rotate it every 0.5s (`popleft()`/`append()`) and write the bit to `male.ring`. Read side: `hardware/female/search/read_pattern/__init__.py`'s `ReadPattern` samples the female's light sensor as a boolean, buffers samples, and tries every sub-step sample offset and circular rotation of each candidate against `colloquy.light_patterns` (majority vote per bin, tolerating `max_mismatches` bit errors) to decode which male + requested drive state she's facing.

**Not currently wired into the live app**: `hardware/female/search/__init__.py`'s `Search.loop()`/`setup()` (which would start `read_pattern`) both unconditionally `raise NotImplementedError("use the turn_back_and_forth thread")` — females move via `Female.turn_back_and_forth` instead, and nothing currently starts `ReadPattern`'s thread automatically. `ReadPattern` can still be started/stopped standalone via its own web-UI node for isolated testing. Also note `colloquy/tests/test_light_sensor_values/*` test scenarios do **not** exercise this decode logic — they log/plot raw analog light-sensor values and threshold-crossing pulse durations, a proxy for body-alignment/visibility timing, not pattern-decode accuracy. `test_with_everything_moving` specifically holds male rings at a **constant steady ON** (not blinking any pattern) — treat its results as "how long is a female facing a lit male," not "can she read his pattern."
