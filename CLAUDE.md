...
The root application declares what the page may offer it: `is_simulated` gates the simulated-hardware panel, `has_hardware` gates the emergency stop (the link *and* the route — an emergency stop cuts torque on nine servos, so an application driving nothing must not pretend to have one). `colloquy/ui/mock.py` answers False to both.

**The page carries no styling.** Every inline `style=` and the one `<style>` block were taken out deliberately, to start the UI work from bare markup — what is left is structure plus the hooks to hang CSS on: `name="..."` attributes on every div (`server commands`, `navigation`, `split`, `commands`, `title`, `open`, `name`) and the classes the markdown and timeline renderers already emit (`md-content`, `timeline-row`, `timeline-time`, …). The only stylesheet still linked is uPlot's, which its charts need to draw at all. **Note the EMERGENCY STOP link is no longer red** — it is a plain link next to `shutdown` until something styles it again.
