# -*- coding: utf-8 -*-
# Source code/Python/pytest_tests/hardware/test_arduino_link.py

"""Opening the Arduino's link, and refusing to open a broken one -
colloquy/hardware/arduino/__init__.py.

Arduino cannot be constructed here (it builds thirty-odd child nodes and
reaches for `colloquy.virtual_hardware`), so every test below calls the
...
from colloquy.hardware.arduino import Arduino, firmware
...
def test_a_board_that_answers_at_no_rate_is_reported_with_the_usb_bus(monkeypatch):
    ...
    from colloquy.hardware.arduino import boards
