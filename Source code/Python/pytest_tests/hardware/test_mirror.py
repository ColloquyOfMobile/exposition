"""Unit tests for colloquy.hardware.mirror.Mirror.
...
from colloquy.hardware.mirror import Mirror
...
def make_mirror(stub_factory, id_number=1, origin=0, position=0):
    ...
    hardware = stub_factory(u2d2=SimpleNamespace(dxls={f"mirror{id_number}": dxl}))
    female = stub_factory(
...
def test_a_mirror_costs_no_bus_traffic_until_somebody_asks(stub_factory):
    ...
    mirror.snapshot_as_child(path=("hardware", "female1", "mirror1"))
