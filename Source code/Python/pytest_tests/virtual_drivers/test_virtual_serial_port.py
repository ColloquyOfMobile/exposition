"""Unit tests for colloquy.virtual_drivers.virtual_serial_port.
VirtualSerialPort - the simulated stand-in for the Arduino's pyserial
connection, used by Arduino.send() (colloquy/drivers/arduino/__init__.py)
...
from colloquy.drivers.angle.conversion import REDUCTIONS, degrees_to_ticks
...
def test_get_nearest_male_uses_male_dxl_ids_7_and_8(stub_factory):
    ...
    # dxl_list[6]/dxl_list[7] in U2D2._dxls (colloquy/drivers/u2d2/
...
def _facing(female, male, bar_at=None):
    ...
    by default)."""
    from colloquy.virtual_drivers.virtual_serial_port import (
        FEMALE_DXL_IDS,
        MALE_DXL_IDS,
    )
...
def test_a_round_trip_costs_time_by_default(stub_factory, monkeypatch):
    ...
    import colloquy.virtual_drivers.virtual_serial_port as module
