...
| 8.7 | Who she answers | she filters by her own drive state: looking for O, she accepts only `I_O`, `I_OP`, `II_O`, `II_OP` and ignores a male asking for P (`Logic_fem.ino:110-225`) | `ReadPattern` decodes "who is it" with no reference to her own drives; nothing filters or acts | Her appetite currently has no bearing on whether a male interests her |
...
4. Decide what should happen when `read_pattern.last_match` finds a hit —
   right now a match is only logged/recorded, nothing satisfies the
   matched drive, so the loop in §7.1 has no closing link yet. TJ closes
   it over the sound channel with a drive-state filter (8.6, 8.7); decide
   whether to port that or to close the loop over light instead.
