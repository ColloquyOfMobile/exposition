...
### Where each pin is on the board

**Only `J11` and `J12` have their names printed.** `J3`, `J4`, `J8`, `J9`
and `J10` are hidden on the silkscreen, so at the rack the table below
names headers you cannot see. This drawing puts the same pins where they
are - front of the board, the Mega's USB socket at the top, to scale - and
writes the signal beside each one.

![The Mega and the signal on every header pin around it](/static/hardware/mega-pinout.svg)

**A body's microphone** comes in on `J11`'s **odd** pin, and the Mega's
analog input for it is the **even** pin beside it:

| Body | Wire from the body | Mega input | Cable |
|---|---|---|---|
| female1 | `J11` 1 | `J11` 2 = A0 | `J5` pin 6 |
| female2 | `J11` 3 | `J11` 4 = A1 | `J1` pin 6 |
| female3 | `J11` 5 | `J11` 6 = A2 | `A-J3` pin 3 |
| **male1** | **`J11` 7** | **`J11` 8 = A3** | **`A-J3` pin 13** |
| male2 | `J11` 9 | `J11` 10 = A4 | `B-J4` pin 10 |

`J11` pin 1 is the square pad, at the top of the connector, level with
the Mega's `A0`.

Regenerated by `py export_mega_pinout.py` at the repository root, out of
`electronic box.kicad_pcb`. Every position, label and cable is read from
the board file. Only the crossed-out tracks are not, because the copper
still has them: they come from `dirty rework` section 3.
