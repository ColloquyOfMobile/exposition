...
**One thing is still missing and it is named in the row: the mirror
(9.6).** The satisfaction moment (9.9) is built as of 2026-08-30 - rhythm,
her light, both duration tables - with only its fifteen *pitches*
unported, for a reason that is the firmware's rather than this port's. It is
the more serious of anything left, because it is the physical medium of the whole
...
| 9.9 | Satisfaction | When the shared drive falls below the interested floor it is **zeroed** and a 6s (120-tick) moment begins, during which neither drive climbs. The male plays a 15-note melody (`act_satisfaction_vals`, note lengths per male in `act_satisfaction_Durations_I/II`, both summing to exactly 120 ticks); the female plays the same rhythm as a brightness ramp in the shared appetite's colour | **Built** (`drivers/satisfaction/`), bar the pitches. Both duration tables are carried whole and both sum to 120 ticks; she takes her rhythm from her *partner's* table and ramps her whole body dark-to-full across each note in the shared appetite's colour. What is not ported is the tune: a body here has one pitch, fixed by which timer drives its pin, so a male keys his own tone to the fifteen lengths instead. Giving it back is a firmware change - OCR per note rather than one per body |
