...
| 1.2 | Search never stops itself when the male becomes satisfied again | `Male.loop()` only *starts* search; nothing in `loop()`, `search.loop()`, or `blink.loop()` re-checks `is_satisfied()` — only `Male.setdown()` calls `search.stop()` (`drivers/male/__init__.py:166-168`) | Once started, a male keeps swaying/blinking indefinitely regardless of drive state, until the whole male thread shuts down | none | ⚠️ Gap (likely unintended — worth confirming with the artist/installer whether this is by design) |
