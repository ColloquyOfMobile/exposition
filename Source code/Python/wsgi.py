...
def make_wsgi(colloquy, shutdown_event, restart_event):
    ...
    def wsgi(environ, start_response):
        try:
            ...
            status, headers, content = parse(*args, colloquy=colloquy, **kwargs)
...
def parse(*args, colloquy, **kwargs):
    app_href = Path("app")  
    iframe_href = Path("iframe")  
    
    str_args = ", ".join(args)
    print(f"parse({str_args})")
    
    handlers = {
        "shutdown": handle_shutdown(event=kwargs["shutdown_event"]),
        "restart": handle_restart(
            events=(
                kwargs["shutdown_event"],
                kwargs["restart_event"],
            )
        ),
        "app": handle_app(
            colloquy=colloquy, 
    ...
    if args:  
        ...
        status, headers, content = handlers["app"]()
...
def handle_app(colloquy, app_href, iframe_href, build_html=build_html, **kwargs):
    def handler(*args):
        content_type = 'text/html'
        status = '200 OK'
        headers = [("Content-Type", content_type)]
...
def handle_iframe(colloquy, app_href, iframe_href, build_iframe=build_iframe):
    def handler(*args):
        content_type = 'text/html'
        status = '200 OK'
        headers = [("Content-Type", content_type)]
...
def handle_ui2(colloquy, app_href, iframe_href):
